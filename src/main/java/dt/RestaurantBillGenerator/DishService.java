...
public class DishService
{
    ...
    public void fillDishList()
    {
        ...
        dishList.add(dish1);dishList.add(dish2);
        dishList.add(dish3);dishList.add(dish4);
        dishList.add(dish5);dishList.add(dish6);
        dishList.add(dish7);dishList.add(dish8);
        dishList.add(dish9);dishList.add(dish10);
    }
    public void showDishMenu()
    {
        ...
        {
            System.out.printf("%-3s %-20s   %-5s Lira\n",dish.getKod(),dish.getIsim(),dish.getFiyat());
        }
    }
    public Dish urunbul(int kod)
    {
        if (kod==0)    {System.out.println("-----Ana menüye yönlendiriloysunuz.entere basın");}
        else
        {
            for (Dish dish:dishList)
            {
            if(dish.getKod()==kod) return dish;
            }
            System.out.println("ürün bulunamadı");
        }
        return new Dish(0,"geçersiz yemek",0);
    }
